...
## Directory Structure
Make sure the following directories exist before running the script. (All path are relative from the directory of the local repository.)
...
### MATALB GUI
Open MATLAB. 
Run the following commnad in the command window.
```
NLCE_add
```

## Input
All paths are relative to the local repository directory.
### Experimental data
...
For this repository, data for a **2D square lattice** provided.
...
| coefficients | `/NLCE/NLCE 2D coefficients/coefficientsOfGraphs<n>.txt` |Stores the corresponding non-zero coefficients (integer-values) of the graphs, $k(c_n)$ and their subgraphs, $k^n(c_m)$ for $m=1, \cdots, n-1$).
...
| u | Spin-dependent Hubbard $U$. For three-flavors $[U_{12},U_{13}, U_{23}]$.| 
