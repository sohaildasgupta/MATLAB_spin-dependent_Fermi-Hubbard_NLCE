...
dmu = 1;
mumax = -(m - 2.5);
%shift the half-filling at mu=0
mu = zeros(1,m);
% for idx = 1:m
%     for jdx = 1:m
%         if idx ~= jdx
%             s = min(idx,jdx);
%             b = max(idx,jdx);
%             mu(idx) = mu(idx) - 0.5 * u((s-1)*m - uint8(s*(s-1)/2) + b-s);
%         end
%     end
% end
% clear('b','s');

muq = linspace(-12,40,50); % balanced mus. can be changed for unbalanced case.
T = 2;
mu_file = join(['../data/csv_files/N=',num2str(m),'/muq.csv']);
T_file = join(['../data/csv_files/N=',num2str(m),'/T.csv']);
writematrix(muq,mu_file);
writematrix(T,T_file);
...
for order=order0   
    ...
    for k = 1:numel(g)
        ...
        itemlist = {'density','density2','doubleoccupancy','fileio','correlator',side1,side2};
        try
        ...
        try
            ...
            nlce = join([nlce, " t=", double(t), "u=", double(u), "n=", n, "m=", m, "D=", dno, 'LDA.mat'],' ');            load(nlce);
            load(nlce);
            if mulist~=muq || Tlist~=T %WARNING : This deletes previous data. Need a better check.
            ...
            for mu0 = muq 
             ...
             %  CTlist(end + 1, :) = [T mu0 densityaccum doaccum energyaccum entropyaccum cnnaccum' ninjaccum'];
                alist(end + 1, :) = [densityaccum(:)' density_sq_accum(:)' doaccum(:)' energyaccum entropyaccum cv_accum niniaccum(:)' ninjaccum(:)']; 
            end
        nlce = join(["../data/mat_files/NLCE_mat_files/N=",num2str(m),"/" ,key],'');
    ...
    for idx = 1:m
        writematrix(CTlist(:, idx)', join(['../data/csv_files/density_m'  num2str(m) '_flav' num2str(idx) '_u' num2str(double(u)) '_' num2str(order) '_dno' num2str(dno) '_n' num2str(n)  '_nlce.csv']));
        writematrix(1/T*(CTlist(:, m+idx))', join(['../data/csv_files/compressibility_m'  num2str(m) '_flav' num2str(idx) '_u' num2str(double(u)) '_' num2str(order) '_dno' num2str(dno) '_n' num2str(n)  '_nlce.csv']));
    end
    writematrix(CTlist(:, 2*m + 1)' - (sum(CTlist(:,1:m),2).^2)', join(['../data/csv_files/total_density_fluc_m'  num2str(m) '_u' num2str(double(u)) '_' num2str(order) '_dno' num2str(dno) '_n' num2str(n)  '_nlce.csv']));
    for idx = 1:uint8(m*(m-1)/2)
        connected = CTlist(:,rev_pair_idx(idx,1)).*CTlist(:,rev_pair_idx(idx,2));
        writematrix(CTlist(:,2*m+1+idx)', join(['../data/csv_files/doublon_m'  num2str(m) '_pair' num2str(idx) '_u' num2str(double(u)) '_' num2str(order) '_dno' num2str(dno) '_n' num2str(n)  '_nlce.csv']));            
        writematrix(CTlist(:,2*m+1+uint8(m*(m-1)/2)+3+idx)'-connected', join(['../data/csv_files/nini_m'  num2str(m) '_pair' num2str(idx) '_u' num2str(double(u)) '_' num2str(order) '_dno' num2str(dno) '_n' num2str(n)  '_nlce.csv']));            
        if order == 1
            connected = 0;
        end
        writematrix(CTlist(:,2*m+1+2*uint8(m*(m-1)/2)+3+idx)', join(['../data/csv_files/ninj_m'  num2str(m) '_pair' num2str(idx) '_u' num2str(double(u)) '_' num2str(order) '_dno' num2str(dno) '_n' num2str(n)  '_nlce.csv']));
    end
    writematrix(CTlist(:,2*m+1+ uint8(m*(m-1)/2) + 2)', join(['../data/csv_files/entropy_m'  num2str(m) '_u' num2str(double(u)) '_' num2str(order) '_dno' num2str(dno) '_n' num2str(n)  '_nlce.csv']));
    writematrix(CTlist(:,2*m+1+ uint8(m*(m-1)/2) + 3)', join(['../data/csv_files/cv_m'  num2str(m) '_u' num2str(double(u)) '_' num2str(order) '_dno' num2str(dno) '_n' num2str(n)  '_nlce.csv']));
end
clear('all');
% delete('*.mat'); % Only for test purposes. Comment it out during actual runs.


%% Plots

t = 1; u = [10 10 10]; 
%need to write a module to make relevant plots with su(n)-symmetric
%variables.
m = 3;
n = -1;
dno = -1;
% order0 = 1:3;
orderlist = [1 4 5];
obslist = {'density','ninj'};


for obs_idx = 1:length(obslist)
    obs = obslist{obs_idx}; 
    save=true;
    impbalance_sun_plots(t,u,m,n,dno,obs,orderlist,save);
end
clear('all');

function impbalance_sun_plots(t,u,m,n,dno,obs,orderlist,save)
    T = 2; %need to check if the correct T data is plotted
    ...
    for order=orderlist
        ...
        for idx = 1:len
            ...
            observable(:,idx) = readmatrix(fname);
